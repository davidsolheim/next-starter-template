...
describe("gallery source", () => {
  ...
  test("admin lives under media and upload reuses /api/upload", () => {
    ...
    const editor = read("components/admin/gallery-album-detail.tsx")
    expect(editor).toContain('htmlFor="album-title">Title')
    expect(editor).toContain('id="album-title"')
    expect(editor).toContain('htmlFor="album-slug">Slug')
    expect(editor).toContain('id="album-slug"')
    expect(editor).toContain('htmlFor="album-description">Description')
    expect(editor).toContain('id="album-description"')
    expect(editor).toContain('htmlFor="album-sort-order">Sort order')
    expect(editor).toContain('id="album-sort-order"')
    expect(editor).toContain("Cover (first item if empty)")
