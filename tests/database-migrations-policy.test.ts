...
describe("database migrations policy", () => {
  ...
  test("gallery_albums migration is committed on media_assets", () => {
    expect(existsSync(join(root, "drizzle/0008_gallery_albums.sql"))).toBe(true)
    const sql = read("drizzle/0008_gallery_albums.sql")
    expect(sql).toContain('CREATE TABLE "gallery_albums"')
    expect(sql).toContain('CREATE TABLE "gallery_album_items"')
    expect(sql).toContain("gallery_album_status")
    ...
    expect(sql).toContain('REFERENCES "public"."media_assets"("id")')
    expect(sql).toMatch(/ON DELETE cascade/i)
    expect(sql).not.toContain("gallery_photos")
    expect(sql).not.toContain('"galleries"')
    expect(sql).not.toContain("org_id")
    expect(read("drizzle/meta/_journal.json")).toContain("0008_gallery_albums")
    const index = read("lib/db/schema/index.ts")
